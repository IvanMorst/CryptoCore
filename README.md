...
rm -rf venv         # удалить старый venv
```
...
### Тестирование поточной обработки файла
...
```bash
python cryptocore.py --algorithm aes --mode ctr --encrypt --key 00112233445566778899aabbccddeeff --input example.txt --output document_encrypted.pdf.cfb.enc

python cryptocore.py --algorithm aes --mode ctr --decrypt --key 00112233445566778899aabbccddeeff --input document_encrypted.pdf.cfb.enc --output document_decrupt.txt
...
Простейший режим шифрования

Принцип: Каждый блок шифруется независимо с одним ключом

Преимущества: Простота, параллелизация

Недостатки: Уязвим к анализу шаблонов (одинаковые блоки → одинаковый шифротекст)

Использование: Только для шифрования случайных данных (не для текста/изображений)
````
...
Принцип: Каждый блок XOR-ится с предыдущим зашифрованным блоком

Инициализация: Требует вектора инициализации (IV)

Преимущества: Скрывает шаблоны данных

Особенность: IV должен быть уникальным для каждого шифрования

Формат: IV (16B) + шифротекст
````
...
Принцип: Преобразует блочный шифр в поточный

Особенность: Позволяет шифровать данные любой длины без padding

Обратная связь: По шифротексту (самоисправляющийся)

Использование: Для поточных данных (сеть, аудио)
````
...
Принцип: Генерирует ключевой поток независимо от данных

Преимущества: Ключевой поток можно предварительно вычислить

Обратная связь: По выходу шифра

Особенность: Нет распространения ошибок
````
...
Принцип: Каждому блоку присваивается уникальный счетчик

Преимущества: Параллельное шифрование/дешифрование

Особенность: Счетчик должен быть уникальным для каждого блока

Использование: Высокая производительность, основа для GCM
````
